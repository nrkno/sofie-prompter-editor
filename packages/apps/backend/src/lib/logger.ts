...
import { stringifyError, isEmpty } from '@sofie-prompter-editor/shared-lib'
...
export function setupLogger(
	...
	if (logPath) {
		...
		if (isProduction) {
			...
			const customFormat = printf((o: Winston.Logform.TransformableInfo) => {
				let str = `[${o.level}]`
				const meta: Partial<Winston.Logform.TransformableInfo> = { ...o }
				delete meta.level
				delete meta.message
