...
import { getOpenedXKeysPanels, requestXkeysPanels, setupXkeysPanel, TrackballValue, XKeys } from 'xkeys-webhid'
...
export class TriggerHandlerXKeys extends TriggerHandler {
	...
	async initialize(triggers?: TriggerConfig[]): Promise<void> {
		...
		if (!(window as any).xkeysInitialized) {
			;(window as any).xkeysInitialized = true
			;(window as any).xkeysConnectedPanels = this.connectedPanels
			...
			if (this.neededPanelIds.size > 0) {
				...
				this.emit('requestXkeysAccess')
			}
		} else {
			this.connectedPanels = (window as any).xkeysConnectedPanels
		}
		...
		for (const xkeys of this.connectedPanels) {
			...
			xkeys.on('jog', (index, value) => {
				const action = this.getAnalogAction('jog', index, value)
				if (action) this.emit('action', action)
			})
			xkeys.on('rotary', (index, value) => {
				const action = this.getAnalogAction('rotary', index, value)
				if (action) this.emit('action', action)
			})
			xkeys.on('shuttle', (index, value) => {
				const action = this.getAnalogAction('shuttle', index, value)
				if (action) this.emit('action', action)
			})
			xkeys.on('tbar', (index, value) => {
				const action = this.getAnalogAction('tbar', index, value)
				if (action) this.emit('action', action)
			})
